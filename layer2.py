...
class SequenceLayer(torch.nn.Module):
    def __init__(self,args,item_embedding_layer):
        ...
        self.args=args
        self.fc_user=nn.Sequential(nn.Linear(args.embedding_size*3, args.embedding_size),
                                    nn.ReLU(True),
                                    nn.Linear(args.embedding_size,args.embedding_size)
                                )
        self.fc_item=nn.Sequential(nn.Linear(args.embedding_size, args.embedding_size*2),
                                    nn.ReLU(True),
                                    nn.Linear(args.embedding_size*2,args.embedding_size)
                                )
        self.PRLoss=torch.nn.BCELoss()
        self.RNN=torch.nn.GRU(input_size=self.args.embedding_size,hidden_size=self.args.embedding_size,batch_first=True)
    ...
    def forward(self, batch,is_eval):
        ...
        sampled_click_emb=self.token(sampled_click).reshape(-1,self.args.embedding_size)
        if not is_eval:
            x=x.view(-1,x.shape[-1]) #B*LxH
            x=self.fc_user(x)
            pos=self.token(pos).view(-1,self.args.embedding_size) #B*LxH
            neg=self.token(neg).view(-1,self.args.embedding_size) #B*LxH
        if is_eval:
            x=x[:,-1,:] #B*H
            x=self.fc_user(x)
            pos=self.token(pos).reshape(-1,self.args.embedding_size) #B x H
            neg=self.token(neg).reshape(-1,self.args.embedding_size)#B*negNum x H
...
class GraphLayer(torch.nn.Module):
    def __init__(self,args,userEmbeddingLayer,itemEmbeddingLayer):
        ...
        self.GCNS=torch.nn.ModuleList(HeterLightGCNLayer(self.op_edges) for i in range(args.n_gcn_layers))
        self.n_layers=self.args.n_gcn_layers
        self.g_out_dim=self.args.embedding_size*1
    ...
    def graph_computer(self,block,h):
        ...
        for layer in range(self.args.n_gcn_layers):
            gcn=self.GCNS[layer]
    ...
    def remove_graph_edges(self,block,user_id,pos_items,sampled_click=None,sampled_click_user_id=None):
        ...
        if sampled_click is not None and self.args.remove_click_edges==1:
    ...
    def remove_graph_inner_edges(self,block):
        ...
        src,dst=src.to(self.args.device),dst.to(self.args.device)
    ...
    def forward(self,blocks,is_eval=False,constra_b=None,have_cart=None,seq_tensor=None):
        ...
        if not self.args.no_constra: # if has constra loss computer it
            ...
            for etype in self.etypes:
                ...
                for ntype in ['user','item']:
                    h[etype][ntype]=self.emb[ntype](torch.arange(0,block.num_nodes(ntype),device=self.args.device))
        ...
        for ntype in ['user','item']:
            ...
            g_cat[ntype]=self.emb[ntype](torch.arange(0,block.num_nodes(ntype),device=self.args.device))
